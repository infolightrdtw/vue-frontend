...
| `$dgMaster.value.exportWord({param})`                        | 針對**目前選取的某一筆**數據以 Word 格式套表（後端 `type='word'`，param 定義見下）                                                                                                  |
| `$dgMaster.value.exportWordPdf({param})`                     | 同 `exportWord` 但輸出 PDF（等同 `exportWord({...param, fileType:'pdf'})`）                                                                                     |
| `$dgMaster.value.exportWordLoop({param})`                    | **整批套表**：依目前查詢條件，把每一筆 master（含明細）各自套一份合併輸出（後端 `type='wordLoop'`，param 同 `exportWord`，另吃 `whereItems`）                                                     |
| `$dgMaster.value.exportWordAll({param})`                     | **整份套表**：依目前查詢條件把全部資料套進同一份 Word（後端 `type='wordAll'`，param 同 `exportWordLoop`）                                                                            |
| `$dgMaster.value.exportPDF({param})`                         | 套用報表版型直接輸出 PDF（後端 `type='pdf'`）。清單 grid 用可見欄位當欄；master-detail（有 `editForm`）則主表/明細欄位取自連動的 DataForm 與其明細 grid                                                  |
| `$dgMaster.value.exportExcel({param})`                       | 將資料輸出至 Excel 套表（後端 `type='excel'`，param 請參考 `exportWord`）                                                                                                |
...
#### exportWord / exportWordPdf / exportWordLoop / exportWordAll / exportExcel 的 param 參數

- `fileName`（或 `wordName`）：套表用的來源範本檔名（省略時後端用 `<頁面名>.docx`）
- `fileType`：輸出的檔案類型，如 `'doc'` 或 `'pdf'`
- `directOpen`：PDF 時是否直接在新分頁開啟（true）而非下載（false）
- `downloadName`：指定下載檔名（省略時用頁面名）
- `password`：PDF 密碼保護（在此設定密碼）
- `whereItems`：（僅 `exportWordLoop` / `exportWordAll`）覆寫整批匯出的查詢條件；省略時沿用 grid 目前查詢條件

> ⚠️ **下載機制**：以上方法都把資料 POST 到後端 `file` 端點（`mode=exportFile`）取得產生的檔名，再導向 `/file?q=<檔名>&n=<下載名>` 由後端 `Content-Disposition: attachment` 觸發下載。**dev/preview 需在 `vite.config.js` 代理 `/file`**（否則 GET 會 fall-through 到 SPA，下載到的會是 index.html 被存成 `file.txt`）。
...
| `$dfMaster.value.setColumnStyle(field, style)` | 設定（或清除）**單一欄位容器**的樣式，常用於條件式背景色。`style` 傳 CSS 字串或物件；傳空字串／`null` 清除。走 reactive，即時重繪（內層 input 會自動透明，背景色才看得到）。**注意**：這是改 reactive 狀態而非直接改 DOM，不能用 `document.querySelector(...).style` 之類戳 DOM 的寫法（Vue 重繪會蓋回去） |
...
1. **autoseq[field, numDig, startValue, step, prefix?]**：依目前資料（grid rows）的同欄位最大序號 +step、左補 0 到 numDig 位，**由前端即時計算**。常用於明細序號，也可用於需要前端可見流水號的欄位。
   - **autoseq['次序',2,1,1]**: 既有最大次序 +1，補滿 2 位（`01`/`02`/…）
   - **autoseq['編號',3,1,1,'A']**: 帶前置碼 `A`，輸出 `A001`/`A002`/…（既有值若為 `A001` 會剝掉前置碼取數字比大小，再補回前置碼）
   - 既有值帶非數字前綴（如 `A001`）時，即使沒指定 prefix 也會自動取尾端數字來接續，避免 `parseInt('A001')=NaN` 一直從起始值重來。

注意事項:
- 當有鍵值欄位欲進行**後端**自動編號時，為了配合後端 AutoNumber 組件的處理，該欄位務必設定預設為 `constant['自動編號']`（畫面新增時會顯示佔位字串「自動編號」），存檔時由後端 AutoNumber 依其前置碼（`GetFixed`，如 `_'A'yyyyMM`）+ 流水號產生真正的編號。
  - ⚠️ **前置碼存在後端**：AutoNumber 的前置碼設定不會傳到前端，所以新增當下畫面只會顯示「自動編號」佔位字串、**無法在前端預覽真正的前置碼+號碼**——真正的編號要存檔後才由後端套上。
  - 若需要「新增時前端就看得到帶前置碼的流水號」，請改用 `autoseq[...]`（前端計算，見上）；代價是流水號由前端依目前畫面資料推算，不像後端 AutoNumber 有 SYSAUTONUM 交易鎖，多人同時新增可能撞號。兩種擇一，不要同時用。

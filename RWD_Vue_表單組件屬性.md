...
1. **autoseq[field, numDig, startValue, step, prefix?]**：依目前資料（grid rows）的同欄位最大序號 +step、左補 0 到 numDig 位，**由前端即時計算**。僅用於明細表的序號欄位（非鍵值、不走後端 AutoNumber）。
   ...
   - ⚠️ **鍵值自動編號請勿用 autoseq**：鍵值一律用 `constant['自動編號']` 走後端產生（見下方注意事項），前端自行推算會撞號。

注意事項:
- 鍵值欄位的自動編號**一律走後端 AutoNumber 組件**，該欄位預設值固定設為 `constant['自動編號']`：
  - **新增 / 存檔時**：畫面欄位顯示佔位字串「自動編號」；送出後由後端 AutoNumber 依其前置碼（`GetFixed`）+ 流水號（`SYSAUTONUM`，有交易鎖）產生真正的編號並寫入資料庫。
  - **顯示時**：直接讀資料庫裡已產生好的編號（含前置碼）即為正確值——前端不需要、也不應該自己組編號。
  - ⚠️ **不要在前端自創流水號**（例如用 autoseq 去推算鍵值）。鍵值的前置碼與流水號一律由後端產生，前端只負責：新增時放「自動編號」佔位字串、顯示時讀回資料庫的值。

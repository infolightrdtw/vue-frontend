...
```js
const dgMaster_onBeforeLoad = (param) => {
...
```js
const dgMaster_onSelect = (index, row) => {
    if ($dfMaster.value) {
...
格式為：`$('id').datagrid(MethodName, Parameters..)`

| 名稱與參數 | 說明 |
|---|---|
| `datagrid('getRows')` | 傳回 `rows`，取得 datagrid 的所有資料（本頁），rows 為 array |
| `datagrid('getTotal')` | 傳回 `row`，取得加總的 row 對象，加總值為 `row.columnName` |
| `datagrid('beginEdit', index)` | 進入單筆的編輯狀態 |
| `datagrid('endEdit')` | 結束編輯狀態 |
| `datagrid('cancelEdit')` | 取消編輯狀態 |
| `datagrid('getSelectedIndex')` | 傳回 `index`，取得目前選中 datagrid 的 index 次序 |
| `datagrid('getSelected')` | 傳回 `row`，取得目前選中 datagrid 的資料，可透過 `row.ColumnName` 取得欄位內容值 |
| `datagrid('select', index)` | 設定選中某一筆資料，index 為次序 |
| `datagrid('check', index)` | 勾選某一筆資料，index 為次序 |
| `datagrid('uncheck', index)` | 取消勾選某一筆資料，index 為次序 |
| `datagrid('checkAll')` | 全部勾選 |
| `datagrid('uncheckAll')` | 全部取消勾選 |
| `datagrid('getChecked')` | 傳回 `rows`，取得所有勾選的資料，rows 為 array 多筆資料 |
| `datagrid('setWhere', whereStr)` | 設定後端數據 Where 的條件，whereStr 為 Where 條件；此方法已包含資料重載，不用再呼叫 `datagrid('load')` |
|  | 注意：`setWhere` 若在 `document.ready()` 中使用，請配合 `AlwaysClose=True`，避免與預設條件衝突 |
| `datagrid('load')` | 重新載入後端數據。若已執行 `datagrid('setWhere', whereStr)`，就不需要再執行此 `load` |
| `datagrid('load', {page: n})` | 指定第 n 頁，重新載入後端資料 |
| `datagrid('loadData', data)` | 根據 data 的內容重新載入資料，data 格式請參考附錄 1 |
| `datagrid('openQuery')` | 打開查詢視窗 |
| `datagrid('openMove')` | 打開整批挑選資料（ClientMove 組件）的視窗 |
| `datagrid('viewRow', index)` | 打開查看對應的 DataForm 表單內容 |
| `datagrid('insert_row')` | 新增資料 |
| `datagrid('copy_row')` | 拷貝目前筆資料 |
| `datagrid('edit_row', index)` | 編輯指定的資料；index 不設定代表目前筆資料 |
| `datagrid('delete_row', index)` | 刪除指定的資料；index 不設定代表目前筆資料 |
| `datagrid('appendRow', row)` | 以 row 的內容來新增一筆資料，row 格式為 `{field:value, ...}` |
| `datagrid('updateRow', {index:index, row:{}})` | 更改指定某一筆資料，row 格式為 `{field:value, ...}` |
| `datagrid('submit')` | 回存資料至後端資料庫 |
| `datagrid('getToolItem', name)` | 傳回 object，取得工具按鈕（ToolItems）的 button 對象，name 為按鈕執行的 function 名稱；可進行 `.hide()` / `.show()` 操作。已知 name：Add/Edit/Delete/Query/Copy/Ok/Cancel/Export/Print 等 |
| `datagrid('showColumn', name)` | 指定顯示某一欄位，name 為欄位名稱 |
| `datagrid('hideColumn', name)` | 指定隱藏某一欄位，name 為欄位名稱 |
| `datagrid('getEditorValue', name)` | 取得目前編輯中某一欄位內容，name 為欄位名稱 |
| `datagrid('setEditorValue', {field:field, value:value})` | 設定目前編輯中某一欄位內容 |
| `datagrid('getColumnOption', Column)` | 取得指定欄位屬性，Column 為欄位名稱 |
| `datagrid('setColumnTitle', {field:field, title:title})` | 設定 datagrid 欄位標題抬頭 |
| `datagrid('importExcel')` | 導入 Excel 數據並回存 |
| `datagrid('importExcelNotApply')` | 導入 Excel 數據但不回存（讓 User 可檢查後再按 `submit` 回存） |
| `datagrid('export')` | 將資料直接輸出到 Excel |
| `datagrid('exportWord', {param})` | 針對某一筆數據以 Word 格式進行套表（param 定義見下） |
| `datagrid('exportWordPdf', {param})` | 針對某一筆數據以 PDF 格式進行套表（param 請參考 `exportWord`） |
| `datagrid('exportExcel', {param})` | 將資料輸出至 Excel 套表（param 請參考 `exportWord`） |
| `datagrid('exportReport')` | 配合 ReportName 屬性印表，格式為 CloudReport 的報表格式 |
| `closest('div').parent().hide()` | 將 datagrid 隱藏 |
...
- 取值：`$('id').datagrid('getEditorValue','name')`（name 為欄位名稱）
- 設值：
...
1. 若使用 formatter 事件又希望以原來的 format 顯示，可用：  
   `const custom_formatter = (value, row, index) => {
       if (value === '隱藏') return '';
...
```js
const dgMaster_上傳檔案_formatter = (value, row, index) => {
  if (!value) return '';
  return {
    type: 'link',
    text: value,
...
- `$('queryobj_id').form('query')`：執行 QueryColumn 查詢功能  
- `$('queryobj_id').form('clear')`：清空查詢條件  
- `$('queryobj_id').form('close')`：關閉查詢視窗  

當 QueryMode 為 Dialog 時：

- `$('queryobj_id'+'_panel').collapse('hide')`：隱藏查詢視窗  
- `$('queryobjid'+'_panel').collapse('show')`：打開查詢視窗  
...
```js
const dfMaster_onApplyError = (message) => {
...
格式為：`$('id').form(MethodName, Parameters..)`

| 名稱與參數 | 說明 |
|---|---|
| `form('open', {row:r, status:s, keys:k})` | 開啟 dataform：r 傳入 row 資料，s 狀態（`inserted` 新增 / `updated` 更改 / `view` 查看），k 鍵值。例如：`{row: {'EmployeeID':'自動編號','Name': 'Andy Kao'},status: 'inserted',keys:'EmployeeID'}` |
| `form('close')` | 關閉 dataform。若要關閉表單，請使用 `self.parent.closeCurrentTab();` |
| `form('status')` | 傳回 dataform 的編輯狀態：`inserted` / `deleted` / `updated` / `view` |
| `form('setWhere', whereStr)` | 設定 dataform 的資料條件 |
| `form('openQuery')` | 打開查詢窗口 |
| `form('insert_row')` | 新增 |
| `form('edit_row')` | 編輯 |
| `form('delete_row')` | 刪除 |
| `form('view_row')` | 查看 |
| `form('cancel')` | 取消編輯 |
| `form('validate')` | 對所有欄位進行檢驗 |
| `form('getDefaultValues')` | 執行 default 組件上所有欄位的預設值動作 |
| `form('submit')` | 存檔 |
| `form('exportWord')` | 印表輸出到 Word |
| `form('exportReport')` | 印表輸出到 PDF |
| `form('hideColumn', ColumnName)` | 隱藏指定欄位 |
| `form('showColumn', ColumnName)` | 顯示指定欄位 |
| `form('optoins')` | 讀取或設定屬性，如：`$('id').form('options').RemoteName='xxxx'`（設定資料來源） |
| `form('loadRow', row)` | 載入一筆 row 資料，row 格式 `{field:value,...}` |
| `form('getRow')` | 讀取目前筆資料；`row.columnName` 為指定欄位內容 |
| `hide()` | 將 dataform 隱藏（通常用於免登入資料輸入完畢時，在 `onApplied()` 事件中使用） |
...
```js
const tabMaster_onSelect = (index, title) => {
...
1. **隱藏頁籤**：`const hideTab = (index) => { Tabs1_tabs.value[index].hidden = true; };`（tabID：頁籤 ID，index：第幾個頁籤）
2. **顯示頁籤**：`const showTab = (index) => { Tabs1_tabs.value[index].hidden = false; };`
3. **切換到指定頁籤**：`const goToTab = (index) => {
       if ($Tabs1.value) {
...
```js
const Tabs1_onSelect = (index, title) => {
    if (index === 0) {
    } 
};
```
...
| Width | int | 寬度 |
| OnBeforeLoad | string | 事件 `onBeforeLoad(param)`：載入資料前觸發。可改變 `param.whereStr` 重新載入 whereStr 指定條件資料。注意：若動態嵌入，必須配合延時的 `tree('load')` 重新載入才會生效 |
...
| onRenderNode | string | 事件 `onRenderNode(row)`：每個節點顯示事件，可自訂節點內容並 `return` 回傳；若節點要有 checkbox 或 button，請綁定 onclick 到 function。checkbox 案例可配合 `tree('renderNode')` 更新節點內容（見下） |
...
```js
const updateNodeCheckbox = (nodeId, isChecked) => {
...
格式為：`$('id').tree(MethodName, Parameters..)`

| 名稱與參數 | 說明 |
|---|---|
| `tree('options')` | 讀取或設定屬性，如：`tree('options').remoteName=資料來源;`（動態設定資料來源） |
| `tree('load')` | 載入資料 |
| `tree('loadData', data)` | 指定 data 載入資料，data 格式請參考附錄 1 |
| `tree('setWhere', WhereStr)` | 設定 tree 的過濾條件 |
| `tree('insert_row')` | 新增一筆資料（須配合 EditForm 設定） |
| `tree('delete_row')` | 刪除目前節點資料 |
| `tree('getSelected')` | 傳回目前被選中的 Node 節點；`object.row` 為選中的 Node 資料 |
| `tree('renderNode', class, {fieldName:value})` | 自訂顯示 Tree 格式時，若欄位值改變須以此方法更新節點資料；class 為自定義 class，fieldName 為欄位名稱，value 為更新欄位值 |
...
| OnBeforeLoad | string | 事件 `onBeforeLoad(param)`：載入資料前觸發，可改變 `param.whereStr` 重新載入指定條件資料；`return false` 可不載入資料。注意：若動態嵌入，需配合延時 `datagrid('load',true)` 重新載入才會生效 |
...
```js
const Schedule1_onRenderItem = (event) => {
...
格式為：`$('id').schedule(MethodName, Parameters..)`

| 名稱與參數 | 說明 |
|---|---|
| `schedule('options')` | 可讀取或設定屬性，如：`schedule('options').remoteName=資料來源;`（動態設定資料來源） |
| `schedule('load', true)` | 載入資料 |
| `schedule('loadData', data)` | 指定 data 載入資料（data 格式請參考附錄 1） |
| `schedule('insert_row')` | 新增一筆行事曆資料 |
| `schedule('setWhere', whereStr)` | 設定行事曆的資料條件；若是打開行事曆就要設定條件，請改用 `schedule('options').onBeforeLoad` 事件 |
...
格式為：`$('id').clientmove(MethodName, Parameters..)`

| 名稱與參數 | 說明 |
|---|---|
| `clientmove('options')` | 可讀取或設定屬性，如：`clientmove('options').remoteName=資料來源;`（動態設定資料來源） |
| `clientmove('openMove')` | 打開 ClientMove 的 datagrid 介面 |
| `clientmove('addAll')` | 自動選取 ClientMove 所有資料並直接進行新增 |
...
| OnFormat | string | 事件 `onFormat(value,row)`：針對要追蹤的欄位進行顯示格式化；格式化後不用再自行調用 `drilldown('open')`，系統會自動處理 |
...
格式為：`$('id').drilldown(MethodName, Parameters..)`

| 名稱與參數 | 說明 |
|---|---|
| `drilldown('options')` | 可讀取或設定屬性，如：`drilldown('options').targetRemoteName =資料來源;`（動態設定資料來源） |
| `Drilldown('options').pageTitle = '我的資料追蹤標題';` | 設定 pageTitle |
| `drilldown('open', row)` | 開啟 drilldown 對象，會根據 mode 決定如何打開；row 為 drilldown 來源資料（示例見下） |

#### drilldown('open', row) 使用示例

來源為 datagrid：

```js
const openDrilldownFromGrid = () => {
...
格式為：`$('id').promptdialog(MethodName, Parameters..)`

| 名稱與參數 | 說明 |
|---|---|
| `promptdialog('show', { onOK: function(row){}, onCancel: function(){} } )` | 開啟 PromptDialog。onOK：按下確定時執行；onCancel：按下取消時執行；row 代表輸入欄位資料，可用 `row.ColumnName` 取得欄位值 |
...
格式為：`$('id').linechart(MethodName, Parameters..)`

| 名稱與參數 | 說明 |
|---|---|
| `linechart('options')` | 可讀取或設定屬性，如：`linechart('options').remoteName=資料來源;`（動態設定資料來源） |
| `linechart('setWhere', WhereStr)` | 設定 linechart 的過濾條件 |
| `linechart('load')` | 重新載入資料 |
| `linechart('loadData', data)` | 根據 data 內容重新載入資料（data 格式請參考附錄 1） |
| `linechart('resize')` | 重新調整統計圖大小（需配合 `setTimeout(...,200)`） |
...
格式為：`$('id').barchart(MethodName, Parameters..)`

| 名稱與參數 | 說明 |
|---|---|
| `barchart('options')` | 可讀取或設定屬性，如：`barchart('options').remoteName=資料來源;`（動態設定資料來源） |
| `barchart('setWhere', WhereStr)` | 設定 barchart 的過濾條件 |
| `barchart('load')` | 重新載入資料 |
| `barchart('loadData', data)` | 根據 data 內容重新載入資料（data 格式請參考附錄 1） |
| `barchart('resize')` | 重新調整統計圖大小（需配合 `setTimeout(...,200)`） |
...
格式為：`$('id').piechart(MethodName, Parameters..)`

| 名稱與參數 | 說明 |
|---|---|
| `piechart('options')` | 可讀取或設定屬性，如：`piechart('options').remoteName=資料來源;`（動態設定資料來源） |
| `piechart('setWhere', WhereStr)` | 設定 piechart 的過濾條件 |
| `piechart('load')` | 重新載入資料 |
| `piechart('loadData', data)` | 根據 data 內容重新載入資料（data 格式請參考附錄 1） |
| `piechart('resize')` | 重新調整統計圖大小（需配合 `setTimeout(...,200)`） |
...
```
 ...
 - 例如：
```js
const setFieldReadonly = (dynamicFieldName, isReadonly) => {
...
格式為：`$('id').combobox(MethodName, Parameters..)`

| 名稱與參數 | 說明 |
|---|---|
| `combobox('load', data)` | 重新載入資料；若有設定 data 代表動態載入資料，data 為資料內容（rows），格式請參考附錄 1 |
| `combobox('setWhere', WhereStr)` | 設定 combobox 的過濾條件，WhereStr 為 Where 條件 |
| `combobox('options')` | 讀取或設定屬性，如：`combobox('options').remoteName=資料來源;`（動態設定資料來源） |
| `getText()` | 取得 combobox 的 textField 顯示內容 |
| `getValue()` | 取得 combobox 的 valueField 內容值 |
| `setValue(value)` | 設定 combobox 欄位內容值 |
...
格式為：`$('id').refval(MethodName, Parameters..)`

| 名稱與參數 | 說明 |
|---|---|
| `combobox('load', data)` | 重新載入資料；若設定 data 代表動態載入資料，data 為資料內容（rows），格式請參考附錄 1 |
| `refval('setWhere', WhereStr)` | 設定 refval 的過濾條件 |
| `refval('getWhereItems')` | 取得 WhereItem 條件，傳回 `whereItem[]`（array） |
| `refval('options')` | 讀取或設定屬性，如：`refval('options').remoteName=動態資料來源;` |
| `refval('doColumnMatch')` | 進行 ColumnMatch 欄位帶值動作 |
| `getText()` | 取得 refval 的 textField 顯示內容 |
| `getValue()` | 取得 refval 的 valueField 內容值 |
| `setValue(value)` | 設定 refval 欄位內容值 |
...
格式為：`$('id').selectoptions(MethodName, Parameters..)`

| 名稱與參數 | 說明 |
|---|---|
| `selectoptions('load', data)` | 重新載入資料；若設定 data 代表動態載入資料，data 為資料內容（rows），格式請參考附錄 1 |
| `selectoptions('options')` | 讀取或設定屬性，如：`selectoptions('options').remoteName=資料來源;` |
| `selectoptions('setWhere', WhereStr)` | 設定 option 的過濾條件 |
| `selectoptions('addItems', value)` | 設定 options 內容；value 為 object，格式 `{text:…, value:…}` |
| `getText()` | 取得 options 的 textField 顯示內容 |
| `getValue()` | 取得 options 的 valueField 內容值 |
| `setValue(value)` | 設定 options 欄位內容值 |
...
格式為：`$('id').datebox(MethodName, Parameters..)`

| 名稱與參數 | 說明 |
|---|---|
| `datebox('options')` | 讀取或設定屬性，如：`datebox('options').format = 'yyyy.mm.dd'`（日期格式設定） |
| `getValue()` | 取得欄位內容值 |
| `setValue(value)` | 設定欄位內容值 |
...
格式為：`$('id').datetimebox(MethodName, Parameters..)`

| 名稱與參數 | 說明 |
|---|---|
| `datetimebox('options')` | 讀取或設定屬性，如：`datetimebox('options').format = 'yyyy.mm.dd'`（日期格式設定） |
| `getValue()` | 取得欄位內容值 |
| `setValue(value)` | 設定欄位內容值 |
...
格式為：`$('id').timebox(MethodName, Parameters..)`

| 名稱與參數 | 說明 |
|---|---|
| `timebox('options')` | 讀取或設定屬性，如：`timebox('options').minHour = 8;`（最小為 8 點鐘） |
| `getValue()` | 取得欄位內容值 |
| `setValue(value)` | 設定欄位內容值 |
...
格式為：`$('id').switch(MethodName, Parameters..)`

| 名稱與參數 | 說明 |
|---|---|
| `switch('options')` | 讀取或設定屬性，如：`switch('options').style='button'` |
| `getValue()` | 取得欄位內容值 |
| `setValue(value)` | 設定欄位內容值 |
...
const dfMaster_照片_onBeforeUpload = (param, fileName) => {
...
```js
const dfMaster_照片_onSuccess = (res) => {
...
格式為：`$('id').fileupload(MethodName, Parameters..)`

| 名稱與參數 | 說明 |
|---|---|
| `fileupload('editImage')` | 編輯圖檔 |
| `fileupload('options')` | 讀取或設定屬性，如：`fileupload('options').folder=指定目錄;`（動態目錄） |
| `fileupload('getUrl')` | 取得檔案 URL 地址 |
| `getValue()` | 取得欄位內容值 |
| `setValue(value)` | 設定欄位內容值 |
...
格式為：`$('id').signature(MethodName, Parameters..)`

| 名稱與參數 | 說明 |
|---|---|
| `signature('reset')` | 清除簽名重來 |
| `signature('replay')` | 顯示簽名軌跡 |
| `signature('options')` | 讀取或設定屬性，如：`signature('options').Height=指定高度;`（動態高度） |
| `fileupload('getUrl')` | 取得檔案 URL 地址 |
| `getValue()` | 取得欄位內容值（為二進位內容） |
...
針對 `loadData` 中的 `data` 參數如透過 Server Method 取得，則須經過 `$.parseJSON()` 轉換，如下：

```js
import { ref } from 'vue';
const $dgMaster = ref(null);

const loadGridData = (data) => {
...
6. `$.callMethod(serverModule,methodName,param,function(result){ ... })`：調用 Server 端非同步程序（Server Method）
   ...
   - 若回傳內容為 table 資料，請用 `$.parseJSON()` 轉成 rows
...
13. `rows $.parseJSON(json)`：將 json 資料轉成 rows 集合
...
const changeGlobalTheme = (themeName) => {
...
const openFlowForm = (formName, tabTitle, workflowName, keyField) => {
    // 1. 產生 8 碼隨機 ID 

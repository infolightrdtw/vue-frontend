...
### 2026-06-11 — 支援 Core Views/Main 的 SYS 系列畫面（在 Vue 主框架看不到 → 改走 Core RenderPage iframe）

**情境**：使用者回報 Core 專案 `Views/Main/` 下的 SYS 系列畫面（Sysusers/Sysgroups/Sysmenus/Sysorg/Sysparas/Sysmessage/Sysswitch/Sysagent/Systeminfocard）在 Vue 主框架點側欄選單後看不到（空白）。

**根因**：Vue 主框架（[Main.vue](src/pages/Main.vue)）每個選單項都以 iframe 開啟，URL 由 [TabPaneIframe.vue](src/html/TabPaneIframe.vue) 組成 `/bootstrap/{solution}/{form}`——這是 **SPA 路由**，會去 `src/pages/bootstrap/{solution}/` 找已移植的 `.vue` 頁面。但 SYS 系列**從未移植成 Vue**，只有 Core 後端 `Views/Main/*.cshtml` 的 jQuery Razor view，由 [MainController.RenderPage](file:///c:/Work/Core_N/EEPWebClient.Core/Controllers/MainController.cs#L191)（route `bootstrap/{page}`，[Startup.cs:292](file:///c:/Work/Core_N/EEPWebClient.Core/Startup.cs#L292)）以**單段**路由服務。⇒ SPA 路由 `/bootstrap/{solution}/Sysusers` 對不到任何 `.vue` → NotFound/空白。對照原 jQuery 主框架 [bootstrap.infolight.main.js `getMenuUrl`](file:///c:/Work/Core_N/EEPWebClient.Core/wwwroot/js/infolight/bootstrap.infolight.main.js#L1937) 本來就是組 `bootstrap/{form}`（單段、不帶 solution）走 RenderPage。

**修法**：
- **前端**（[TabPaneIframe.vue](src/html/TabPaneIframe.vue)）：`src` computed 新增判斷 `^sys`（比照後端 RenderPage 的 `page.ToLower().IndexOf("sys")==0`）。命中時 iframe src 改回 `/bootstrap/{form}{query}`（單段、不帶 solution）→ 命中 Core RenderPage；其餘維持 `/bootstrap/{solution}/{form}`（已移植 RWD 頁面走 SPA）。Core-hosted build 同源直接可用。
- **dev/preview 代理**（[vite.config.js](vite.config.js)）：SYS Razor view 還需 Core 動態端點與資料 API，但 SPA 自己也擁有 `/main`(主框架) 與 `/bootstrap/{solution}/...`，故用 **bypass 依方法/段數精準分流**（抽成 `sysPageProxy` 同掛 server+preview）：`GET /bootstrap/<單段 sys 開頭>`→Core、其餘 `/bootstrap`→SPA；`POST /main` 或 `/main/*`→Core（SYS 頁 loadData/updateData/file，比照 [bootstrap.infolight.js](file:///c:/Work/Core_N/EEPWebClient.Core/wwwroot/js/infolight/bootstrap.infolight.js#L628) 打 `../main`）、`GET /main`→SPA；`/scripts`(locale)、`/stylesheets`(main.css)→Core。**`/js/**` 不代理**（vue-frontend `public/js` 已自帶 jQuery/bootstrap 資產，避免影響 SPA 執行期動態載入）。bypass 回傳 `req.url` 即略過代理交還 SPA（vite 官方 pattern）。

**範圍外（刻意不動）**：
- **AgentSetting** 已有 Vue 版（[Main.vue:329 `agentSetting()`](src/pages/Main.vue#L329) 用 `component: AgentSetting`，系統下拉開啟），故 detection 只取 `^sys` 不含 agentsetting，避免搶掉 Vue 版。
- **switchUser**（Sysusers 內）會 POST `../logon`，dev 未代理 `/logon`（怕影響 SPA 登入流程）→ dev 下切換使用者可能不完整；Core-hosted build 同源正常。

**驗證**：`npm run build` 通過（純前端改 TabPaneIframe；vite.config 只影響 dev/preview，不進 bundle）。**通則**：選單項若是 Core 後端 jQuery 畫面（SYS 系列、未移植 RWD），iframe 走單段 `/bootstrap/{form}` 命中 RenderPage，不要走 SPA 的 `/bootstrap/{solution}/{form}`；dev 用 bypass 把 SYS 相關請求（單段 sys、POST /main、/scripts、/stylesheets）分流給 Core 而不動 SPA 既有路由。

**追加（同日，使用者回報「SYS 畫面顯示成功但沒套用主題顏色」）**：
- **根因**：SYS iframe 是 Core jQuery 頁面（非 Vue），收不到 Vue 主題系統（[theme-base.css](src/assets/stylesheets/themes/theme-base.css) 消費 `--theme-*` 變數 + [vue_*.css](src/assets/stylesheets/themes/) 提供變數）。Main.vue 切主題只 `postMessage THEME_CHANGE`（[App.vue](src/App.vue) 監聽）對 Vue iframe 有效，SYS 頁面沒人處理。SYS 只用 Core 後端傳的 `bootstrap_{RWDThemeID}.css`（且 Core themes 只有 default/black/violet/blue/yellow 5 個，缺 green/indigo/mauve，無法一對一）。
- **修法（純前端，不碰 Core / 不必 rebuild 後端）**：利用 SYS iframe 與主框架**同源**（dev 都 5173、Core-hosted 同源），父視窗直接寫入 iframe.contentDocument：
  - 新增 [sysThemeBridge.ts](src/utils/sysThemeBridge.ts)：`SYS_THEME_VARS`（6 主題 × 7 變數，鏡像 vue_*.css）+ `BRIDGE_CSS`（對 SYS 實際 class 著色：`.datagrid-title`/`.datagrid-btn`/`.modal-header[.bg-primary]`/`.btn-primary`/`.bootstrap-datagrid` 表頭+striped+hover+info+選取/分頁 bs3`li.active>a`+bs5`.page-item`/`.form-control:focus`/`.lcs_switch.lcs_on`/**列命令鈕 `.datagrid-command .btn-link`+其內 `.glyphicon`/`.fa`/`.text-primary`**（檢視/編輯/刪除 icon，原本是 btn-link 藍/text-primary 藍））+ `applyThemeToSysFrame(frame, theme)`（設 `<html>` inline 變數 + 注入 `<style id=vue-theme-bridge>`）+ `getCurrentThemeName()`（讀 `localStorage['user-theme']`）。
  - [TabPaneIframe.vue](src/html/TabPaneIframe.vue)：iframe 加 `ref`、`isSysPage` computed；`frameLoaded` 時若 SYS → 注入目前主題；`onMounted` `emitter.on('themeChange')` 即時重套、`onUnmounted` off。
  - [Main.vue](src/pages/Main.vue) `changeTheme`：除既有 postMessage，新增 `emitter.emit('themeChange', themeName)`（import emitter）讓各 TabPaneIframe 對其 SYS iframe 重新注入。
- **驗證**：`npm run build` 通過。⚠️ 尚未實機驗（需起 Core 44368 + dev 5173，點 SYS 選單看配色 + 切主題即時變色）。**通則**：要讓「同源但非 Vue」的 iframe（Core jQuery 頁）跟著 Vue 主題，父視窗注入 `--theme-*` 變數 + override CSS 到其 document，不要依賴 postMessage（對方沒監聽者）。

### 2026-06-11 — 修 DataGrid 工具列「匯出 Excel」鈕報 `function:'exportExcel' not found`（沒進 TOOL_METHODS）+ 對齊 jQuery 套表匯出

**情境**：使用者點清單工具列「匯出 Excel」鈕，跳 `function:'exportExcel' not found`，要求比照 [bootstrap.infolight.js `exportExcel`](file:///c:/Work/Core_N/EEPWebClient.Core/wwwroot/js/infolight/bootstrap.infolight.js#L5735) 呼叫 API 套表印表。延續 2026-06-05（export）/ 2026-06-04（exportWord 系列）同一類 bug。

**根因**：`exportExcel`（與 `exportReport`）原本只寫成 [DataGrid.vue](src/components/controls/DataGrid.vue) `defineExpose` 裡的 **inline arrow**、**沒放進 `TOOL_METHODS`** → 工具列 `toolItemClick` 查不到 → fall-through 到 `$.invoke('exportExcel')` 當頁面函式 → not found。且舊 inline 版實作有兩個問題：(1) 用**選取列 `masterRow`** 當匯出來源（清單匯出應送目前**查詢條件** whereItems/whereStr，比照 jQuery 用 `opts.whereItems`）；(2) 下載走舊的空 `<a download="">` 機制（就是 2026-06-04 抓到的 file.txt bug，該輪已改用 `triggerFileDownload`），exportExcel/exportReport 漏改。

**修法**（[DataGrid.vue](src/components/controls/DataGrid.vue)）：
- 把 inline `exportExcel:`/`exportReport:` 抽成具名 `async function exportExcel()` / `exportReport()`（放在其他 export* 函式旁），對齊 jQuery + 既有 named 模式：用 `collectQueryFilter()` 取 whereItems/whereStr、`triggerFileDownload()` 下載、`name` 剝 `_Query$`（查詢頁 員工資料表_Query 匯出要用基底模組名 員工資料表，與 jQuery `replace(/_Query$/,'')` 一致）。exportExcel param 補齊 jQuery 的 headRemoteName/detailRows/excelName/sort/directOpen/password/watermark/headTitle/titleName；fileType==='pdf' && directOpen 時走 inline 開窗。
- `TOOL_METHODS` 加 `exportExcel` + `exportReport` → 工具列鈕不再報錯；defineExpose 移除重複 inline（改由 `...EXPOSE_METHODS`→`...TOOL_METHODS` 帶出，contract 仍 reachable）。
- ⚠️ 注意 [visibleToolItems:598](src/components/controls/DataGrid.vue#L598)：readonly grid 會把 TOOL_METHODS 內的鈕**濾掉隱藏**。exportExcel 進 TOOL_METHODS 後，行為與既有 exportWord/export **一致**（readonly 時一起隱藏），非新行為。

**驗證**：`npm run build` 通過；DataGrid 單元 **27 passed**（新增 3：工具列 onclick='exportExcel' 打到 exportFile('excel') 不 fall-through、exportExcel 送 type=excel + 帶 whereStr 不帶 masterRow、name 自動剝 _Query）；contract 133 passed / 8 skipped。**通則**：工具列會用到的 export* 一律要進 `TOOL_METHODS`（不只 defineExpose）；清單匯出送查詢條件不送選取列；下載一律走 `triggerFileDownload`。

### 2026-06-11 — 修 excel 產生的網頁（產品資料表 / 員工資料表_Query）開啟即彈 `Cannot read properties of null (reading 'split')`

**情境**：使用者回報 excel 產生的網頁（產品資料表、員工資料表_Query 等）一打開就彈錯誤對話框（紅字 + 確定鈕）`Cannot read properties of null (reading 'split')`。那個對話框是 [pageApi.ts `showError`](src/utils/pageApi.ts#L101) 把 catch 到的 `e.message` 顯示出來。

**根因**：[DataGrid.vue `loadData`](src/components/controls/DataGrid.vue#L842) 的 `keys = data.keys.split(',')` **沒防 null**。查詢頁（員工資料表_Query）與無主鍵定義的 module（產品資料表）後端 `getDataset`/loadData 回傳 `keys: null` → `null.split(',')` 拋錯 → 被 `load()` 的 try/catch 接住 → `$.showError(e)` 彈出該訊息。對照原 jQuery（[bootstrap.infolight.js:7944](file:///c:/Work/Core_N/EEPWebClient.Core/wwwroot/js/infolight/bootstrap.infolight.js)）本來就是 `var keys = data.keys ? data.keys.split(',') : [];`——Vue 移植時漏了這個守衛。`keys` 在 line 388 本就預設 `[]`，null 時回退空陣列即可（與 DataList.vue:150 的 `typeof data?.keys === 'string'` 守衛、OrgChart `data.keys || []` 一致）。

**修法**（[DataGrid.vue:842](src/components/controls/DataGrid.vue#L842)，一行）：`keys = data.keys ? data.keys.split(',') : []`，比照原 jQuery。檢查全 repo 其餘 `data.keys.split` 只剩 DataList/OrgChart，兩者已有守衛，不需改。

**驗證**：`npm run build` 通過；DataGrid 單元測試 **24 passed**（新增 1：loadData 回傳 `keys: null` 時不呼叫 showError、rows 正常載入）。**通則**：所有 `data.keys.split(',')` 一律先判 truthy（查詢頁 / 無主鍵 module 會回 null），回退 `[]`。

### 2026-06-10 — 修起迄查詢（同 field、不同 operator）的查詢欄位 blur 後互相同步、無法個別編輯

**情境**：使用者用「起迄查詢」——兩個查詢欄位共用同一個 `field`（如 `到職日期`），只靠 `Title`/`Operator` 區分（大於 / 小於）。畫面上兩個輸入框 blur 後會互相同步成同一個值，無法個別輸入起、迄。

**根因**：[BEditor.vue:2](src/components/elements/BEditor.vue#L2) 以 `v-model="row[column.field]"` 綁定，查詢表單的 row 是 [DataGrid.vue](src/components/controls/DataGrid.vue) 的 `queryValues`。兩個 queryColumn 共用同一個 `field` → 兩個編輯器綁到 `queryValues['到職日期']` **同一個 key** → 改一個另一個跟著變。

**修法**（[DataGrid.vue](src/components/controls/DataGrid.vue)，純前端）：新增 computed `internalQueryColumns`，對「重複 field」的查詢欄位產生唯一綁定 key（`field__q{index}`），同時保留真實 `_qfield`/`operator`。單一 field 維持原樣（key 即 field）以相容既有行為。
- 兩處 BHtmlForm（Panel 與 Dialog）`:columns` 改綁 `internalQueryColumns`。
- 兩處 whereItems 建構（`load()` 內與 `collectQueryFilter()`）改 iterate `internalQueryColumns.value`，讀值用唯一 key、push 時 `field` 用 `_qfield`（真實欄位）+ 各自的 `operator`。
- `runFuzzyQuery` 同步改用 internal key。順手把 `load()` 內一個 `value !== undefined & value !== ''`（bitwise）修成 `&&`。

**驗證**：`npm run build` 通過；DataGrid 單元測試 **23 passed**（新增 2：兩個共用 field 的查詢編輯器互不同步、`load()` 送出兩筆同 field 不同 operator/value 的 whereItem）。**通則**：查詢表單若多欄共用同一 field（起迄查詢），綁定 key 必須唯一化，但 whereItem 的 `field` 仍送真實欄位 + 各自 operator。

### 2026-06-05 — 修 export 直接匯出 404/畫面空白：後端 API endpoint 根本沒接 exportDataset mode（跨 Core_N）

**情境**：使用者點「直接匯出」後畫面變空白、無檔案，console 報 `file?q=&n=報價單` **404**——注意 `q=` 是空的。延續前面 export()→exportDataset 系列。

**根因（追到後端才現形）**：Vue `dataApi.ts` 的 `exportDataset` POST 到 `/api/ApiMain/file`（[ApiMainController.File()](file:///c:/Work/Core_N/EEPWebClient.Core/Controllers/ApiMainController.cs)），但**那個 endpoint 只硬寫了 `download` 與 `exportFile` 兩個 mode，其餘一律掉進 `else` → `UpLoadFile()`（檔案上傳）**，根本沒把 `exportDataset` 轉給 `FileProvider.ProcessRequest`（ProcessRequest 雖有 exportDataset case，但這條 HTTP 路徑到不了）。上傳 handler 沒檔案 → 回空陣列 `[]` → 前端 `${[]}`＝空字串 → `q=` 空 → 後端 `/file` 404 → `triggerFileDownload` 的 `window.location.href` 把整個 SPA 導去那個 404 → **空白畫面**。
- 對照原 jQuery（[bootstrap.infolight.js:1168 `exportData`](file:///c:/Work/Core_N/EEPWebClient.Core/wwwroot/js/infolight/bootstrap.infolight.js)）：它 POST 到 **`../main/file`**（`MainController` → 走 `ProcessRequest`，有 exportDataset），所以原本會動。但 `MainController.File` 用 **Referer-based** `AccountProvider.GetClientInfo(Referer)` 認證，Vue SPA 是 session 認證、且 vite proxy 沒代理 `/main`，所以前端不能直接改打 `/main/file`。
- **我先前的疏忽**：2026-06-03 加 exportDataset 時只看到 `FileProvider.ProcessRequest` 有 case 就以為通了，沒檢查 `ApiMainController.File()` 這個實際 HTTP 入口有沒有把 mode 轉進去。

**修法**：
- **後端** [ApiMainController.cs](file:///c:/Work/Core_N/EEPWebClient.Core/Controllers/ApiMainController.cs)（Core_N repo）：`File()` 新增 `mode == "exportDataset"` 分支，比照既有 `exportFile` 分支（建 FileProvider + 設 ClientInfo），改呼叫 `fileProvider.ProcessRequest(param)`（→ ExportXlsx 直接匯出整份資料集，回檔名）。**⚠️ 後端要重新 build + 重啟跑著的後端（44368）才生效。**
- **前端** [DataGrid.vue](src/components/controls/DataGrid.vue) `exportData`：把 `if (!file)` 強化成 `typeof file !== 'string' || !file.trim()`——後端若回非字串/空值（如 `[]`）會跳錯誤提示 return，**不會再用 window.location 把整頁導走變空白**（防呆，real fix 在後端）。

**驗證**：前端 `npm run build` 通過、DataGrid 單元測試 21 passed；後端改動是既有 exportFile 分支的鏡像（未跑整方案 build）。**通則**：Vue 端新增任何 `mode=xxx`，要確認 `ApiMainController` 對應 action（data/file/flow…）真的有把該 mode 轉給 provider 的 ProcessRequest，光看 ProcessRequest 有 case 不夠——HTTP 入口才是真相。

### 2026-06-05 — 修 DataGrid 工具列「直接匯出」鈕報 `function:'export' not found`（export 沒進 TOOL_METHODS）

**情境**：使用者點清單工具列的「直接匯出 Excel」鈕，跳 `function:'export' not found`。延續 2026-06-03 把 `export()` 改走後端 `exportDataset`（直接匯出），但那輪只把 `export` 放進 `defineExpose`，**沒放進 `TOOL_METHODS`**。

**根因**：[DataGrid.vue `toolItemClick`](src/components/controls/DataGrid.vue) 先查 `TOOL_METHODS[item.onclick]`，找不到才 fall-through 到 `$.invoke(item.onclick)` 當成頁面函式 → `export` 不在 TOOL_METHODS（exportWord/exportWordPdf/exportWordLoop/exportWordAll/exportPDF 在 2026-06-04 都註冊了，就漏了 `export` 本體）→ 工具列鈕 onclick='export' 報「找不到函式」。`export` 是 JS 保留字、原本只能寫成 `defineExpose` 裡的 inline `export:` arrow，無法在 TOOL_METHODS（宣告在前）引用。

**修法**（[DataGrid.vue](src/components/controls/DataGrid.vue)）：把 inline `export:` 抽成具名 `async function exportData()`（放在其他 export* 函式旁、TOOL_METHODS 之前），順手改用 `triggerFileDownload`（對齊 2026-06-04 的下載修法，避免 file.txt bug）；`TOOL_METHODS` 加 `export: exportData`（物件 key 可用保留字）；移除 defineExpose 裡重複的 inline `export:`（改由 `...EXPOSE_METHODS`→`...TOOL_METHODS` 帶出）。

**驗證**：`npm run build` 通過；DataGrid 單元測試 **21 passed**（新增：工具列 `onclick='export'` 的鈕點下去會打到 exportData/後端 exportDataset，而非 fall-through 到 `$.invoke('export')`）；contract 133 passed / 8 skipped。**通則**：工具列會用到的 method 一律要進 `TOOL_METHODS`（不只 defineExpose），否則 onclick 會被當頁面函式而報 not found。已 push（fac15e0）。

### 2026-06-04 — 修系統表 relation（SYSGROUP/SYSUSER）grid 顯示空白：loadRelations 抓錯欄位

**情境**：使用者回報 relation 設系統表（如 SYSGROUP）時，RWD(jQuery) datagrid 正常顯示、Vue 不顯示。（延續前一輪 relation trim/反應性修法，但那輪沒抓到這個。）

**根因**：[DataGrid.vue `loadRelations`](src/components/controls/DataGrid.vue#L799) 收集要查詢的值時用 **`r[col.relation.options.valueField]`**——但 `valueField` 是**關聯表**的鍵欄位，不是 grid row 的欄位。實例（員工資料表部門欄）：`field:'部門'`、`relation.options.valueField:'GROUPID'`、`textField:'GROUPNAME'`、`remoteName:'員工資料表.ref_SYS_GROUPS'`。**row 把值存在 `r['部門']`（內容＝GROUPID 值），row 根本沒有 `GROUPID` 欄** → `r['GROUPID']` 是 undefined → 收集到空 → 不查 → 快取空 → 顯示空白。
- **為何一般 relation 沒事**：當欄位名剛好＝關聯表 valueField 時（如客戶編號 relation `valueField:'客戶編號'` 且欄位也叫客戶編號）`r[valueField]` 剛好抓得到，**矇對**。系統表 valueField（GROUPID/USERID）跟欄位名（部門/主管）不同才爆。
- **顯示側本來就用對的欄位**：[BTableCell.vue:27](src/components/elements/BTableCell.vue#L27) 用 `row[column.field]` 當查表值；jQuery `getDisplayRowP` 也是收集 `r[opt.field]`（[bootstrap.infolight.js:1799](file:///c:/Work/Core_N/EEPWebClient.Core/wwwroot/js/infolight/bootstrap.infolight.js)）。所以是 Vue 的 load 側用錯欄位、跟顯示側對不上。

**修法**：[DataGrid.vue loadRelations](src/components/controls/DataGrid.vue#L799) 收集值改成 **`r[col.field]`**（row 實際存的值），與顯示側 + 後端 IN 查詢鍵一致。一行改動。

**驗證**：`npm run build` 通過；DataGrid 單元測試 **20 passed**（新增：relation 欄位收集值取自 `r['部門']` 而非 `r['GROUPID']`，斷言 `loadRelationValues` 被以 `'001,002'` 呼叫；順帶把 flowApi mock 補上 `getFlowWhereItems` 等 stub，讓不帶 alwaysClose 的 mount 能真的跑 load）；全測試 628 passed / 17 failed（baseline gitignored demo 頁）/ 8 skipped。**通則**：grid relation 一律「收集 `r[field]` → 查關聯表 valueField IN → 映射 valueField→textField → 用 `r[field]` 查回 text」，load 與顯示兩側都用 `field`，不要用 valueField 去 row 上抓。

### 2026-06-04 — 修 Relation 欄位顯示空白（trim/反應性/crash）+ autoseq 支援前置碼並修補位 bug

**情境**：使用者回報兩問題：(1) ChatCoder 解析自動編號的前置碼，主鍵自動編號只顯示「自動編號」；(2) 用到 Relation 的欄位無法正常顯示內容。

**問題 2 根因（relation 顯示空白）**：grid 的 relation 欄位走 [BTableCell.vue](src/components/elements/BTableCell.vue) → [Relation.vue](src/html/Relation.vue) → `$.getRelationValue`（讀 [pageApi.ts](src/utils/pageApi.ts) 的 `rValues` 快取，快取由 DataGrid `loadRelations` 預載）。三個 bug：
- **沒 trim**：`getRelationValue` 用 `rValues[remoteName][value]` 直接查，但 EEP/Informix 的 **CHAR 欄位常有尾隨空白**（row 值 `'001  '` vs 關聯表 key `'001'`）→ 對不上 → 顯示空白。原 jQuery 是 `$.trim(value)`（[bootstrap.infolight.js:21245](file:///c:/Work/Core_N/EEPWebClient.Core/wwwroot/js/infolight/bootstrap.infolight.js)）。修：存 key 與查 value 兩邊都 `String(x).trim()`。
- **反應性**：Relation.vue 用解構 props（Vue 3.2 失去反應性），`watch(()=>value)` 永遠不更新。修：改 `const props = defineProps()` + `watch(()=>props.value)`。
- **crash**：`loadRelationValues` 若查詢失敗 `data` undefined，`data.reduce` 在 try 外 → 炸掉整個 grid load。修：`if (!Array.isArray(data)) return`。
- 另：快取 key 由 `remoteName` 改成 `remoteName|valueField|textField`（避免共用 remoteName 不同欄位互相覆蓋）；`seperator`/`separator` 拼字容錯。

**問題 1 根因（主鍵自動編號只顯示「自動編號」）**：查 Core `AutoNumber` 元件（[AutoNumber.cs](file:///c:/Work/Core_N/EEPServerTools.Core/Components/AutoNumber.cs)）——前置碼是後端 `GetFixed`（如 `_'A'yyyyMM`），**只在存檔時套用、設定不會傳到前端**，也沒有 client 預覽 endpoint。所以主鍵用 `constant['自動編號']` 佔位、新增當下只能顯示「自動編號」，**前端無法預覽真正的前置碼+號碼**（這是架構限制，非 bug）。
- **可行的替代 + 順手修 bug**：把前端 `autoseq[field,numDig,start,step]` 規則**加上前置碼**（第 5 參數）並讓它能讀含前置碼的既有值。兩處都改：[pageApi.ts](src/utils/pageApi.ts) 的 `autoseq`（DataGrid 明細走這條，**活路**）與 [DataForm.vue](src/components/controls/DataForm.vue) `computeDefault` 的 autoseq case（一致性；註：DataForm 的 `ctx.rows=props.rows` 其實沒宣告該 prop → 表單 autoseq 形同 dead code，主鍵 autoseq 沒有 rows 來源，所以主鍵前置碼流水號前端算不了）。
- **順手修 pageApi autoseq 補位 bug**：原本 `strValue = '0' + value`（用 raw 數字補位，空 rows 時甚至產出 `0-1`）→ 改 `'0' + strValue`。
- 新行為：`autoseq['編號',3,1,1,'A']` → `A001`/`A002`…；既有值 `A001` 會剝前置碼取數字接續；無 prefix 時也會自動取尾端數字（`parseInt('A001')=NaN` 不再一直從起始值重來）。

**.md**：附錄8 新增 `autoseq[...]`（含前置碼）說明，並改寫「自動編號」注意事項。

**驗證**：`npm run build` 通過；新增 `src/utils/__tests__/pageApi.relation.test.ts`（8 passed：relation trim/查詢失敗不 crash/快取 key 不衝突 + autoseq 前置碼/補位）；DataForm 19 passed。全測試 627 passed / 17 failed / 8 skipped——17 failed 仍為 baseline 既有 gitignored demo 頁，與本次無關。

**✅ 使用者已定案（2026-06-05）**：鍵值自動編號**一律走後端 AutoNumber**——新增/存檔時前端只放 `constant['自動編號']` 佔位、由後端產生真正編號（前置碼 GetFixed + SYSAUTONUM 流水號），**顯示時直接讀資料庫的值**。**前端不要自創流水號**（不要拿 autoseq 推算鍵值，會撞號）。autoseq **僅用於明細表序號**（非鍵值）。附錄8 已照此改寫；prefix 程式碼保留供明細序號用。

### 2026-06-04 — 修 DataGrid `exportWord` 下載成 file.txt + 補全 exportWordPdf/exportWordLoop/exportWordAll/exportPDF 並對齊 Core file API

**情境**：使用者回報 DataGrid `exportWord` 印出的不是 docx 而是 `File.txt`；`exportWordPdf/exportPDF/exportWordLoop/userDefined` 更是直接報錯。要求對齊 Core 的 export API。

**查證（Core 為準）**：
- 下載端點是 `FileController`（`/file?q=<檔名>&n=<下載名>`，[FileController.cs:22-97](file:///c:/Work/Core_N/EEPWebClient.Core/Controllers/FileController.cs)）——**不在 `/api` 底下**。它依檔名副檔名設 `Content-Type` + `Content-Disposition: attachment; filename*=…`。
- 套表入口是 POST `/api/ApiMain/file`（`mode=exportFile`）→ `FileProvider.ExportFile`（[:640-687](file:///c:/Work/Core_N/EEPGlobal.Core/Provider/FileProvider.cs)），`type` 支援 **word / wordLoop / wordAll / excel / report / pdf**。`dataApi.ts` 的 `exportFile(type,id,param)` 早就能打全部 type（只差 Vue 端沒接）。

**兩個根因**：
1. **file.txt**：`exportWord` 下載用「空的 `<a download="">` click」。dev/preview 的 vite proxy **只代理 `/api`，沒代理 `/file`** → GET `/file?q=…` fall-through 到 SPA index.html（HTML）→ 空 download attr 讓瀏覽器拿 URL 路徑 `file` 當檔名 + 嗅內容 → 存成 `file.txt`。（使用者在 5173 dev 看，見 memory `runtime_topology`。）
2. **報錯**：`TOOL_METHODS` 只註冊 `exportWord`，工具列其他匯出鈕（exportWordPdf/exportPDF/exportWordLoop…）的 onclick 找不到方法 → fall-through 到 `$.invoke()` 當成不存在的頁面函式 → 報錯。

**修法**：
- [vite.config.js](vite.config.js)：server + preview 的 proxy 都加 `/file` → 後端 44368（dev/preview 下載才打得到後端）。
- [DataGrid.vue](src/components/controls/DataGrid.vue)：
  - 新增共用 `triggerFileDownload(file, downloadName, inline)`：**改用 `window.location.href`（下載）/ `window.open`（inline）對齊原 jQuery**，靠後端 `Content-Disposition: attachment` 帶檔名，不再用空 download attr。
  - 新增共用 `collectQueryFilter()`（parentObject / flow / queryColumns / whereStr），`export()` 也改用它去重。
  - `exportWord` 清掉重複 `const { exportFile }`、套用新下載；新增 `exportWordPdf`（type=word+pdf）、`exportWordLoop`（type=wordLoop）、`exportWordAll`（type=wordAll）、`exportPDF`（type=pdf，清單 grid 用可見欄位；有 editForm 則主表/明細欄位取自連動 DataForm 與其明細 grid）。
  - **5 個方法全註冊進 `TOOL_METHODS`** → 工具列鈕不再報錯；`...TOOL_METHODS` 自動 expose。
  - expose 加 `columns` / `remoteName` / `whereStr`（供 exportPDF editForm 讀明細 grid 欄位）。
- [DataForm.vue](src/components/controls/DataForm.vue)：expose 加 `columns` / `panelColumns`（供 exportPDF editForm 讀主表/連動欄位）。
- [RWD_Vue_表單組件屬性.md](RWD_Vue_表單組件屬性.md)：DataGrid 方法表補 exportWordLoop/exportWordAll/exportPDF 三行、改寫 exportWord/exportWordPdf 說明；param 小節補 wordName/whereItems + ⚠️「下載機制 + dev 要代理 /file」說明。

**userDefined**：使用者明確要求**先不動**（Core 端是 ~2000 行互動式報表設計器 modal，規模遠大於匯出方法，另案處理）。工具列的 userDefined 鈕暫時仍會報錯。

**驗證**：`npm run build` 通過；DataGrid 單元測試 **19 passed**（新增 6：exportWord 帶選取 row + 無選取不打後端、exportWordPdf=pdf、exportWordLoop 帶 whereStr、exportPDF 帶可見欄位、5 方法皆 expose）；contract 141 passed / 8 skipped。全測試 619 passed / 17 failed / 8 skipped——**17 failed 已 `git stash` 驗證為 baseline 既有**（gitignored demo 頁面對不上 page test，見 2026-06-03/05-21 條目），與本次無關。**通則**：所有 export* 下載一律走 `window.location.href`/`window.open` + 後端 attachment header，不要用空 `<a download="">`；dev/preview 記得代理 `/file`。

### 2026-06-04 — 新增 DataForm `setColumnStyle`（條件式欄位背景色）+ 釐清 FieldOnBlur 日期公式陷阱 + 修 dataApi.ts 既有 build breaker

**情境（員工請假單頁）**：使用者兩個問題：(1) 請假天數（FieldOnBlur 公式 `EndDate-StartDate+1`）算不出來、欄位空白且像不能編輯；(2) 「請假 > 1 天底色變黃」AI 用 `dgMaster_rowStyler` 做、grid 也沒效果。延續前一輪「Vue 改狀態不是改 HTML」的觀念題。

**問題 1 根因（FieldOnBlur 純 JS 運算）**：[FieldOnBlur.vue:100-148](src/components/controls/FieldOnBlur.vue#L100-L148) 用 `new Function` 算公式。`EndDate-StartDate+1` 對日期**字串**相減 → `Number('2026/06/08')=NaN` → 結果 NaN → 空白。且 FieldOnBlur 是 `deep watch currentRow`（[:24-31](src/components/controls/FieldOnBlur.vue#L24-L31)），手動輸入會被重算覆蓋（所以「不能編輯」）。**正解公式**：`Math.round((new Date(EndDate)-new Date(StartDate))/86400000)+1`（`new` / `Date` token 因 currentRow 無同名欄位不會被當欄位，安全）。已在 [.md FieldOnBlur 段](RWD_Vue_表單組件屬性.md#L1131) 補日期差範例 + deep watch 覆寫說明（原範例只有純數字 `合計未稅*0.05`，這是 AI 寫錯公式的源頭）。

**問題 2（兩個畫面分開看）**：
- **Grid（rowStyler）功能本來就在**（[DataGrid.vue:91](src/components/controls/DataGrid.vue#L91)、[:502-517](src/components/controls/DataGrid.vue#L502-L517) 含 `--bs-table-bg` 覆蓋）。沒效果是**問題 1 連動**：LeaveDays 一直空 → `Number(row.LeaveDays)>1` 永遠 false。修好公式 + 有 >1 的資料即生效（LeaveDays 在 grid `hidden:true` 不影響 rowStyler 讀資料）。
- **Form 欄位著色缺功能**：DataForm 無「設欄位樣式」API，editor 各自算 inputStyle（[Numberbox.vue:64](src/components/editors/Numberbox.vue#L64)）不吃外部 style。**新增 `setColumnStyle(field, style)`**（比照 `setColumnReadonly`）：[DataForm.vue](src/components/controls/DataForm.vue) 加 `columnStyles = reactive({})`、template 欄位容器綁 `:class df-col-styled` + `:style`、scoped CSS 讓內層 `.form-control/.form-select` 透明（背景才看得到）、expose。.md DataForm 方法表加一行。頁面 `dfMaster_onLoad` 用 `watch(currentRow.LeaveDays)` 呼叫之。

**順手修 build breaker**：[dataApi.ts:154](src/utils/dataApi.ts#L154) 有一個 stray 全形 `。`（夾在 2026-06-03 exportDataset commit b14473c，working tree clean = 已 commit 進去），`npm run build` 直接 `Unexpected "。"` 失敗。移除後 build 通過。**⚠️ 表示 main 上的 build 其實是壞的**（與前一輪日誌「build 通過」矛盾，可能那輪沒實跑 build 或在別份 copy 跑）。

**驗證**：DataForm 單元測試 **19 passed**（新增 setColumnStyle：set→帶 class+style、clear→移除）；contract 133 passed / 8 skipped；`npm run build` 通過。**通則**：條件式欄位/列著色一律走 reactive——清單列用 DataGrid `rowStyler`，表單欄位用 DataForm `setColumnStyle`，都不可戳 DOM。日期相減公式必須用 `new Date()`，不能直接減字串。

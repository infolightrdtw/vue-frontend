...
> 本機實測：2026-05-08 — **build + preview + 全測試通過**

### 累積成果（vs 起點）
| 指標 | 起點 | 報告現在 | 本機實測 (2026-05-08) |
|---|---|---|---|
| 測試檔案 | 26 | 57+ | 57 passed |
| 單元/頁面/契約 案例 | 210 | 600 passed + 23 todo + 8 skip | **623 passed / 8 skipped** |
| E2E specs | 0 | 33 passed（real backend strict） | 未跑（需 backend） |
| 覆蓋率 | n/a | 56.53% statements | 未量 |
| Production deps 漏 | 7 | 0 | — |
| Spec ↔ code 不一致 | 28 | 23（KNOWN_GAPS） | — |
| 會 crash 的 release blocker | 4 | 0 | — |
| **Production build** | 失敗 | 未驗 | **✅ 通過、無 warning** |
| **Preview smoke**（4 頁 + Logon + /） | — | — | **✅ 6/6 HTTP 200、0 real errors** |
...
- [ ] **release 前手動驗 4 張 demo 頁面**（CLAUDE.md `## 6. 上線最短路徑` 步驟 4）— 需 backend 起來後做 CRUD smoke。

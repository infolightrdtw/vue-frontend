...
**.md**：附錄8 新增 `autoseq[...]`（含前置碼）說明，並改寫「自動編號」注意事項——標明 `constant['自動編號']`=後端 autonum 佔位（前端無法預覽前置碼），要前端可見流水號才改用 `autoseq[...]`（代價：前端算、多人同時新增可能撞號）。
...
**⚠️ 待回報使用者**：主鍵自動編號「前置碼前端預覽」架構上做不到（前置碼在後端、不傳前端、無預覽 API）。要嘛接受存檔後才有號（現狀，後端 SYSAUTONUM 有交易鎖最安全），要嘛改用前端 `autoseq[...]`（即時可見但會撞號），要嘛我另外在 Core 加一個「取下一個 autonum 預覽」endpoint（跨 repo、較大）。
